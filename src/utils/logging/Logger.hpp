...
#ifndef OPENASR_UTILS_LOGGER_H
#define OPENASR_UTILS_LOGGER_H
...
#define LOG_TRACE(logger) \
  ...
  else ::openasr::logging::LogStream(::openasr::logging::Trace, Unwrap(logger).module(), __FILE__, __LINE__).stream()
#define LOG_DEBUG(logger) \
  if (!Unwrap(logger).IsDebugEnabled()) {} \
  else ::openasr::logging::LogStream(::openasr::logging::Debug, Unwrap(logger).module(), __FILE__, __LINE__).stream()
#define LOG_INFO(logger) \
  if (!Unwrap(logger).IsInfoEnabled()) {} \
  else ::openasr::logging::LogStream(::openasr::logging::Info, Unwrap(logger).module(), __FILE__, __LINE__).stream()
#define LOG_WARN(logger) LOG_WARNING(logger)
#define LOG_WARNING(logger) \
  if (!Unwrap(logger).IsWarningEnabled()) {} \
  else ::openasr::logging::LogStream(::openasr::logging::Warning, Unwrap(logger).module(), __FILE__, __LINE__).stream()
#define LOG_ERROR(logger) \
  if (!Unwrap(logger).IsErrorEnabled()) {} \
  else ::openasr::logging::LogStream(::openasr::logging::Error, Unwrap(logger).module(), __FILE__, __LINE__).stream()

// We shouldn't really be logging block level information at anything higher than debug.
#define LOG_TRACE_BLOCK(logger) ::openasr::logging::BlockTracer _block_tracer_ ## __LINE__ (Unwrap(logger), ::openasr::logging::Trace, __PRETTY_FUNCTION__, __FILE__, __LINE__)
#define LOG_DEBUG_BLOCK(logger) ::openasr::logging::BlockTracer _block_tracer_ ## __LINE__ (Unwrap(logger), ::openasr::logging::Debug, __PRETTY_FUNCTION__, __FILE__, __LINE__)
...
namespace {                                             \
  ::openasr::logging::Logger& logger()                  \
  {                                                     \
    static ::openasr::logging::Logger instance(module); \
    return instance;                                    \
  }                                                     \
}
 ...
 *   openasr::logging::configure_logging(::getenv("MY_APP_LOGGING_CONFIG"));
 */

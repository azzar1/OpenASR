...
LoggerModulePtr const& LoggerModules::GetModule(std::string const& module)
{
  ...
  auto i = modules_.find(lower_module);
  if (i != modules_.end())
    return i->second;

  // Make the new LoggerModule and its parents.
  // Split on '.'
  auto idx = lower_module.rfind(".");
...
void LoggerModules::Reset()
{
  for (auto module : modules_)
    module.second->SetLogLevel(NotSpecified);
}
...
std::string LoggerModules::DumpLoggingLevels(std::string const& prefix) const
{
  ...
  bool first = true;
  for (auto const moudle :  modules_)
  {
    std::string const& module_name = moudle.first;
    LoggerModulePtr const& module = moudle.second;

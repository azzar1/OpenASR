...
#include "logging/LoggerPrivate.hpp"
...
namespace ot = openasr::testing;

using namespace openasr::logging;
...
TEST(TestLogger, TestModuleName) {
  Logger logger("openasr.logging");
  EXPECT_EQ(logger.module(), "openasr.logging");
}
...
TEST(TestLoggingWriter, TestWriteMessage) {
  ...
  ot::CaptureLogOutput log_output;
